...
| Date | Change | Details |
|------|--------|---------|
| 2026-08-24 | **Price intelligence + morning digest** | New `app/core/price_stats.py` module detects price drops vs 7-day peak (threshold 10%) and identifies listings in cheapest quartile (low-in-search signal, min 5 active). New `app/core/digest.py` module: 24h alert scan with 7-day dedup window; emails via stdlib smtplib + Gmail STARTTLS (multipart); `notified_at` stamped on send success (retry semantics); skips empty digests; ALERTS_ENABLED kill switch. New `./oemparts digest` CLI + 07:00 launchd job (5 agents now). Alerts table now in use (dedup + retry semantics). Dashboard: price-drop + low-in-search badges on listings, Median $ column on searches. New `title_contains_part_number()` helper (number-only match for alerts). Config: SMTP_HOST/PORT/USERNAME/PASSWORD, DIGEST_START_HOUR, DEAL_PERCENTILE (all .env). Test suite: 115 green tests. |
