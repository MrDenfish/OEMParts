...
**Auth architecture:** Designed with a pluggable auth dependency. `basic.py` (hardcoded single user, Phase 1 default) and `clerk.py` (Clerk-hosted auth, implemented 2026-07-14) are both live; `AUTH_BACKEND` selects which, and `get_current_user()` in `dependencies.py` dispatches. Route handlers depend on `get_current_user()` and remain unchanged across the swap.
...
| **eBay Taxonomy API** | Valid Year/Make/Model hierarchy per marketplace | Same pool as Browse | Used to populate cascading dropdowns. Cache full taxonomy locally and refresh weekly. Fitment (Motors parts) category lookups use `marketplace_id=EBAY_MOTORS_US`, not the general `EBAY_US` default — that resolves to the separate eBay Motors category tree (tree id `100`), which is the tree that actually contains parts categories. |
...
6. **Item Specifics enrichment** — once per cycle, `enrich_listing_aspects()` fetches Brand / MPN / OE-OEM Part Number via eBay getItem for listings that never received them (capped at 200/cycle, fail-soft on transient errors, quota logged as `ebay_item` provider in `api_quota_log`). Results stamped in `brand`, `mpn`, `oe_part_number`, `aspects_fetched_at` columns. The AI relevance classifier uses this data to detect off-brand alternatives.
7. **Affiliate URL generation** — when rendering listings in the dashboard, the stored `ebay_item_url` is combined with the EPN campaign ID at display time (or pre-computed and stored as `affiliate_url`).
8. **Dashboard surfaces results** — filterable by vehicle, part, price range, condition, seller rating. Price history charts on click.

### Key Algorithmic Pieces

**Part query construction:** Two modes depending on whether a fitment category was resolved at search creation.
  - **Fitment mode (category known):** A bare query is sent to Browse API with `category_ids` (the resolved eBay leaf category) plus `compatibility_filter=Year:2012;Make:Land Rover;Model:LR4` (semicolon-separated; only works in eBay Motors categories 6028 and descendants). This eliminates the bulk of false positives where a title happens to contain "LR4" but the part doesn't actually fit.
  - **Fallback mode (category unknown or resolution failed):** The descriptive text is enriched with the vehicle's year/make/model — e.g., "water pump" becomes "2012 Land Rover LR4 water pump" — and sent as a bare keyword query without `category_ids` or `compatibility_filter`. This is Phase 1's workaround and is less precise but broader in coverage.
...
### Fetch Schedule (local launchd; EC2 cron later)

Active since 2026-08-24 via macOS launchd LaunchAgents (`scripts/install_schedules.sh`; `scripts/scheduled_job.sh` ensures Docker + Postgres are up and migrations applied before each run; logs to `logs/scheduled.log`). launchd fires a missed run once on wake if the Mac was asleep; runs are skipped only if the Mac is powered off. When EC2 deployment happens, the same commands move to cron in the worker container.

| Time (local) | Command | Purpose |
|-----------|---------|---------|
| 03:00 | `oemparts fetch --cycle=nightly` | Full refresh of all active searches |
| 03:30 | `oemparts cleanup` | Purge expired listings, archive old price history |
| 07:00 | `oemparts digest` | Email the morning deal digest (price drops + notable new listings); includes AI classification step when `AI_FILTER_ENABLED=true` |
| 09:00, 14:00, 20:00 | `oemparts fetch --cycle=intraday` | Refresh high-priority searches only (user-flagged) |
| Sunday 04:00 | `oemparts taxonomy-sync` | Resolve fitment categories for active searches missing one (`--all` re-resolves every active search) |
...
./oemparts taxonomy-sync                                # Resolve fitment categories for active searches missing one
./oemparts taxonomy-sync --all                          # Re-resolve fitment categories for ALL active searches (slow, use after eBay category-tree changes)
...
- **Phase 2 (taxonomy + fitment):** Taxonomy-based compatibility_filter with two-mode fetch (fitment mode + fallback), category resolution at search creation, `taxonomy-sync` CLI, Category column in dashboard, semicolon separator fix, `compatibility_checked` tracking (see Changelog 2026-08-23)
- **Phase 2 (scheduled fetches):** launchd-driven fetch cycles on the owner's Mac — nightly/intraday/cleanup/taxonomy-sync, installed via `scripts/install_schedules.sh` (see Changelog 2026-08-24)

### Next Up (owner's choice — Phase 2 feature scope complete 2026-08-26)
- VIN decoding via NHTSA — **done 2026-08-26** (PR #7; NHTSA vPIC, not Taxonomy API)
- Y/M/M cascading dropdowns — **done 2026-08-26** (PR #7; NHTSA models + curated makes, not Taxonomy API)
- EC2 deployment with Docker Compose, Caddy reverse proxy, SSL for oempartsagent.com — **deferred 2026-08-23** (owner chose local-first for now; the macOS launcher below covers day-to-day use)
- Candidate follow-ups (unstarted): Tier 2 AI photo/description inspection for digest-notable listings; templates-hoist refactor (kills route-module circular imports); `create_search` vehicle-ownership check; per-user digest email + config-driven dashboard-link origin; skip generic "Other" taxonomy categories; user-friendliness pass (onboarding, refresh-all, validation, tooltips)

### Backlog (Phase 3+)
- Email alerts via AWS SES (price drops, new listings) — done in local-first form (Gmail SMTP digest, 2026-08-24); SES swap deferred to EC2 phase
...
| Date | Change | Details |
|------|--------|---------|
| 2026-09-14 | **OEM-only searches fetch deep (limit 200)** | The OEM-only title filter discards most of each page *before* persist, so the default 50-listing fetch starved those searches: the LR124471 alternator search kept 6 of 50 while eBay reported 125 matching New listings. The Browse API allows `limit=200` in a single call — same quota cost as 50. `run_single_search` now passes `limit=FETCH_OEM_DEEP_LIMIT` (new config, default 200) on all three fetch paths when `oem_only` AND `oem_number` are set; other searches keep `FETCH_MAX_LISTINGS_PER_QUERY` (deeper pages are unfiltered noise there). `search_ebay` clamps any limit to eBay's 200 max. Only filter-passing listings are persisted, so DB/AI/staleness load is unchanged. Live-validated: alternator search 6→9 listings, all OEM/Genuine/part-number matches. Deliberately NOT paginating past 200 (`offset`) until a real search exceeds 200 matches. Tests: `tests/core/test_oem_deep_fetch.py` + browse clamp test. Full suite after merging all four 2026-09-14 PRs (#8-#11): 273 passed. |
| 2026-09-14 | **Empty fetch cycles no longer recorded as "failed"** | Every intraday fetch_run since 2026-08-24 showed `status=failed, searches_processed=0`: the intraday cycle fetches only high-priority searches, none exist, and `complete_fetch_run`'s status rule (`errors < searches_processed`) evaluated 0 < 0 → "failed" for a healthy no-op. New rule: failed only when `errors > 0 and errors >= searches_processed` (processed counts successes, so all-searches-erroring still fails; a nothing-to-do cycle completes). Historical mislabeled rows (intraday, 0 processed, 0 errors) relabeled to completed by one-time UPDATE. Tests: `tests/worker/test_fetch_run_status.py` (4 cases incl. end-to-end empty intraday run). Full suite: 239 passed. |
| 2026-09-14 | **Canonicalize vehicle make/model to eBay's fitment-catalog spelling** | Live incident: the Browse API's `compatibility_filter` matches values **case-sensitively** — `Make:LAND ROVER` silently matched 48 listings where `Make:Land Rover` matched 125 (no warning, normal 200), and even `Model:lr4` vs `LR4` loses results. NHTSA VIN decodes return all-caps makes/models, so the VIN-created vehicle (2026-08-26) had silently shrunk all fitment-mode fetches for ~3 weeks. Fix: `get_compatibility_property_values()` in `app/sources/ebay_taxonomy.py` (Taxonomy `get_compatibility_property_values` endpoint, tree 100; new `compat_value_cache` table, migration `b70b7de84943`, 30-day TTL, serve-stale-on-failure) + `canonicalize_make`/`canonicalize_model` in `app/core/compatibility.py` (match on lowercase-alphanumeric key so "LandROVER"/"LAND ROVER"/"land rover" all map to "Land Rover"; unknown values and API failures return input unchanged). Wired into `POST /vehicles` (storage) and `POST /vehicles/decode-vin` (form display), both fail-open. `REFERENCE_FITMENT_CATEGORY_ID = "177697"` is the lookup category (any fitment leaf yields the catalog's make list; verified live — 472 makes). Do NOT query multiple case variants at fetch time — eBay accepts exactly one canonical spelling; normalization at vehicle creation is the fix. Tests: `tests/sources/test_taxonomy_compat_values.py`, `tests/core/test_canonicalize_fitment.py`, + vehicle-route tests (autouse identity stubs keep tests off the network). Full suite: 258 passed. |
| 2026-09-14 | **OEM-only toggle re-applies the filter to linked listings** | Live incident: the OEM-only filter runs only at fetch time (`search_runner`), so 47 aftermarket alternators fetched during a brief toggle-off on the new LR124471 alternator search stayed linked (and on the dashboard) after the toggle went back on. Fix: `remove_non_oem_links(db, search)` in `app/db/queries.py` deletes a search's `search_listings` rows whose listing titles fail `title_matches_oem` (junction rows only — shared `listings` rows and price history untouched; returns 0 when the search has no OEM number). The `PATCH /searches/{id}/toggle-oem-only` route calls it whenever the toggle lands ON with an OEM number set, and logs the pruned count. Tests: `tests/db/test_oem_reapply.py`, `tests/web/test_toggle_oem_only.py` (the latter pins `basic_auth_username` to the test user's email — in basic mode the username IS the tenant identity, so a mismatch correctly 404s). Full suite: 240 passed. |
| 2026-08-26 | **AI query crafting: never guess part numbers** | Live incident: a search created with only `LR072537` as query text (OEM field empty) had its query rewritten by `craft_query` to "Land Rover water pump" — a hallucinated (and wrong) part identity. Fix (rode PR #7): `looks_like_part_number()` in `app/core/ai_relevance.py` detects a bare part-number query and skips crafting entirely; such a query also auto-copies into the empty OEM# field before the oem_only default is computed (so OEM-only defaults ON, matching user intent); the crafting prompt gains an explicit never-guess-part-numbers rule. Also that day: owner deleted their original vehicle during live validation — `ON DELETE CASCADE` removed all 12 searches (by design; the confirm dialog warns). Only the LR072537 compressor search was wanted; it was rebuilt from `logs/scheduled.log` fetch URLs (query/category/condition are recoverable from there). Listings and price history were unaffected. |
| 2026-08-26 | **VIN decoding + Y/M/M cascading dropdowns** | New `app/sources/nhtsa_vpic.py` module: `decode_vin(db, vin)` queries NHTSA vPIC API to decode VIN → Year/Make/Model/Trim with fail-soft semantics (returns None on any failure; vehicle form falls back to free-text). `is_valid_vin(vin)` validates VIN format (17 alphanumeric chars per standard, excluding I/O/Q). `get_models_for_make_year(db, make, year)` retrieves model-name list for a make+year pair. All three use cache-then-API pattern: `vin_decode_cache` table (existing since Phase 1, now actively used; PK on VIN), `nhtsa_model_cache` table (new; migration `27d3dcc8a334`), 30-day TTL on model cache (`MODEL_CACHE_TTL_DAYS = 30`). VINs never logged in full — all log lines use `_redact()` function (first/last 4 chars, e.g., `ABCD…WXYZ`). **Web layer:** `POST /vehicles/decode-vin` (HTMX endpoint) decodes user-entered VIN and returns pre-populated vehicle form or fallback-to-text message. `GET /vehicles/models` (HTMX endpoint) serves model dropdown options for a selected make+year pair. **Vehicle form UX:** Year selector (1981–2027) → Make dropdown (from constant `COMMON_MAKES` in `app/web/makes.py`, ~60 makes) → Model dropdown (fetched via `GET /vehicles/models`). Free-text fields (make_text, model_text) always available and override dropdowns if filled. Vehicle creation route (`POST /vehicles`) accepts both dropdown selections and free-text overrides; final make and model resolved in order: text > dropdown > empty. `is_valid_vin()` gate before storage: VIN stored in `vehicles.vin` only if valid; invalid VINs silently dropped. **No new config / env vars** — `NHTSA_VPIC_BASE_URL` pre-existed (Section 11). **NHTSA calls not logged to api_quota_log** (eBay Browse/Taxonomy only). **Tests:** Full suite: 218 passed. |
| 2026-08-25 | **Brand-aware filtering (Item Specifics enrichment)** | New module `app/sources/ebay_item.py`: `fetch_item_aspects()` calls eBay Browse API getItem endpoint to retrieve Brand / MPN / OE-OEM Part Number as ground-truth item attributes (sellers often game titles; Browse summaries carry no brand data). New columns on `listings` table: `brand`, `mpn`, `oe_part_number` (all nullable strings, max 100 chars), `aspects_fetched_at` (TIMESTAMPTZ, nullable) — migration `cf2692058fc6`. Fetch enrichment: `enrich_listing_aspects()` function in `app/worker/fetcher.py` runs once per cycle (called from `run_fetch_cycle`), fetches up to 200 listings/cycle (cap `ASPECT_CAP_PER_CYCLE`), fail-soft (transient errors return None, trigger retry next cycle; 404/410 treated as final, stored as all-None). Quota logged as provider `ebay_item` in `api_quota_log`. AI relevance classifier (`ai_relevance.py`) gains new `offbrand` verdict: triggered when listing Brand does not match the search query's brand (e.g., search "Dorman OEM part" → listing Brand: Amk → offbrand). Rule: missing/Unbranded Brand never triggers offbrand. Dashboard tag for offbrand verdicts: `alt` (displayed alongside existing `acc` and `off` tags for accessory/unrelated). Rollout includes one-time manual verdict reset for all existing `search_listings` rows (`relevance` → NULL, `relevance_checked_at` → NULL) to trigger re-classification under the updated rules. Test suite: 178 tests passing. |
| 2026-08-24 | **AI relevance filter for listings** | New `app/core/ai_relevance.py` module: one structured-output call per search per day using `claude-opus-5` (or `claude-haiku-4-5` for cost reduction), cap 100 links per search per run, fail-open (errors/refusals/malformed responses leave `relevance` and `relevance_checked_at` NULL). New `classify_pending` step in digest: runs before scanning alerts and demotes accessory/unrelated listings (still counted in other-new). Creation-time AI query crafting (best-effort; user-provided text retained on failure). Dashboard audit tags: `acc` and `off` indicate AI classification state. New `anthropic` dependency in `requirements.txt`. Database: migration `e710aab2051a` adds `relevance` (String(10), values: part/accessory/unrelated/NULL) and `relevance_checked_at` (TIMESTAMPTZ) columns to `search_listings` table. Config: `AI_FILTER_ENABLED` (default `false`), `ANTHROPIC_API_KEY` (pay-as-you-go, separate from Claude subscription), `AI_MODEL` (swap `claude-opus-5` ↔ `claude-haiku-4-5` as needed). Digest schedule (07:00) includes classification when enabled. Test suite: 144 green tests. The kill switch stops new classification; existing verdicts continue to demote alerts and show tags. Classification runs inside the digest job, so ALERTS_ENABLED=false also pauses it. |
| 2026-08-24 | **Price intelligence + morning digest** | New `app/core/price_stats.py` module detects price-drop signal (2-day scan window for digest alerts — spans the nightly fetch cadence — 7-day window for the dashboard badge, 10% threshold) and identifies listings in cheapest quartile (low-in-search, min 5 active). Dashboard shows price-drop + low badges using 7-day lookback; alert dedup window is 7 days. New `app/core/digest.py` module: 2-day alert scan with 7-day dedup window; emails via stdlib smtplib + Gmail STARTTLS (multipart); `notified_at` stamped on send success (retry semantics); skips empty digests; ALERTS_ENABLED kill switch. New `./oemparts digest` CLI + 07:00 launchd job (5 agents now). Alerts table now in use (dedup + retry semantics). Dashboard: price-drop + low-in-search badges on listings, Median $ column on searches. New `title_contains_part_number()` helper (number-only match for alerts). Config: ALERTS_ENABLED, SMTP_HOST/PORT/USERNAME/PASSWORD, DIGEST_FROM, DIGEST_TO, DIGEST_PRICE_DROP_PCT, DEAL_PERCENTILE (all .env). Test suite: 115 green tests. |
| 2026-08-24 | **Scheduled fetch cycles (launchd, local-first)** | Fetches now run hands-off on the owner's Mac. New `scripts/scheduled_job.sh` (launchd entry point: ensures Docker Desktop + db container up, `alembic upgrade head`, then the matching `oemparts` command; logs to `logs/scheduled.log`), `scripts/install_schedules.sh` / `uninstall_schedules.sh` (idempotent; four LaunchAgents `com.mrdenfish.oemparts.*`: nightly 03:00, intraday 09/14/20:00, cleanup 03:30, taxonomy-sync Sun 04:00, local time; missed runs fire once on wake). New shared `scripts/_docker_env.sh`: locates the docker CLI for launchd/Finder contexts — this machine's Docker.app lives in a non-standard folder ("/Applications/Python related/"), so minimal-PATH contexts couldn't find `docker`; this also fixed a latent bug in the OEMParts.app launcher (`launch_local.sh` now sources the same helper). Verified live: kickstarted nightly run processed 8 searches (3 correctly TTL-skipped), 94 listings, 40 new, 0 errors; uninstall/reinstall round trip clean. |
| 2026-08-24 | **Taxonomy fitment merged to `main` (PR #2) + GitHub Actions CI** | Feature branch `phase2/taxonomy-fitment` merged as `2b565d2` after live-API validation (see 2026-08-23 row). New `.github/workflows/ci.yml`: ruff (pinned to the local venv's version — an unpinned newer ruff produced 61 spurious findings), full Alembic upgrade chain from scratch, and the 73-test suite against a `postgres:15` service container with the dedicated `oemparts_test` DB; runs on every PR and push to `main`. New `ruff.toml` excludes Alembic files from the *formatter* only (committed migrations stay byte-identical; lint still covers them). Post-merge data tuning (owner-approved, local DB only): "OEM, Genuine" reset to fallback mode; three hose searches pointed at Coolant Hoses (33601); two wordy searches renamed ("water pump thermostat hose", "expansion tank heater hose" — connector words like "to" narrow Browse API matching even though it does normalize "waterpump"→"water pump") with OEM-only off, immediately yielding 6 and 2 fitment-verified listings on searches that had never matched anything. |
| 2026-08-23 | **Taxonomy-based fitment filtering (Phase 2 core)** | eBay Taxonomy API integration for leaf-level category resolution + two-mode fetch. **Database:** Migration `120604f077fb` adds `category_id` and `category_name` columns to `searches` table. **New modules:** `app/sources/ebay_taxonomy.py` — Taxonomy client with `taxonomy_cache` read-through (caches each category's compatibility properties across all users, 30-day TTL); `app/core/compatibility.py` — `resolve_fitment_category(db, query_text)` resolves at search creation (inline eBay call, sanctioned exception). **Fetch logic:** Two modes in `search_runner.py`: (1) Fitment mode — bare query + resolved `category_ids` + semicolon-separated `compatibility_filter` (only eBay Motors 6028+); (2) Fallback mode — Phase 1 enrichment (prepend vehicle Y/M/M to query) if resolution fails or API rejects fitment call (per-run fallback). **CLI:** `./oemparts taxonomy-sync` now functional with `--all` flag for full refresh (incremental by default). **Web:** New "Category" column in searches table (shows resolved category name or "—" for fallback mode) with tooltips explaining fitment status. **Fixes:** `compatibility_filter` separator changed from comma to semicolon (eBay API requirement). New `listings.compatibility_checked` boolean (written during fetch to track which listings had fitment applied). **Tests:** New test `test_searches_page_shows_category_column` validates Category column display; full suite green with 69 tests. **Live-validation correction (2026-08-24):** category resolution was failing for all searches because the tree lookup used the default `EBAY_US` marketplace; fitment categories live in the separate eBay Motors tree, resolved via `marketplace_id=EBAY_MOTORS_US` → tree id `100`. |
| 2026-08-23 | **macOS app-style local launcher** (EC2 deployment deferred) | Owner chose to stay local-first for now, emulating the Recoup Tracker project's launch pattern. New `setup.sh` (repo root): idempotent one-command setup — builds/updates `.venv`, installs deps, builds `~/Applications/OEMParts.app` + Desktop shortcut. The bundle delegates to new `scripts/launch_local.sh`: starts Docker Desktop if needed, brings up the `db` container, waits for `pg_isready`, runs `alembic upgrade head`, starts uvicorn on port 8000 under a pidfile (`.run/uvicorn.pid`, logs to `logs/app.log`), opens the browser; idempotent (re-run just opens the browser). Errors surface via macOS dialog when launched from the bundle. New `scripts/stop_local.sh` stops uvicorn + db container. `.gitignore` adds `logs/`, `.run/`. `.env` unchanged (`AUTH_BACKEND=basic`); Clerk stays merged but dormant. Verified: cold launch (Docker auto-start → dashboard 302 login redirect), idempotent relaunch, clean stop. iPhone-app idea parked as Phase 3+ (thin client over hosted API; pairs with push alerts — see session notes). |
