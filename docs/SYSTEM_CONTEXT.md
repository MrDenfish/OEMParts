...
| Date | Change | Details |
|------|--------|---------|
| 2026-08-25 | **Brand-aware filtering (Item Specifics enrichment)** | New module `app/sources/ebay_item.py`: `fetch_item_aspects()` calls eBay Browse API getItem endpoint to retrieve Brand / MPN / OE-OEM Part Number as ground-truth item attributes (sellers often game titles; Browse summaries carry no brand data). New columns on `listings` table: `brand`, `mpn`, `oe_part_number` (all nullable strings, max 100 chars), `aspects_fetched_at` (TIMESTAMPTZ, nullable) — migration `0c4ef8c18f29`. Fetch enrichment: `enrich_listing_aspects()` function in `app/worker/fetcher.py` runs once per cycle (called from `run_fetch_cycle`), fetches up to 200 listings/cycle (cap `ASPECT_CAP_PER_CYCLE`), fail-soft (transient errors return None, trigger retry next cycle; 404/410 treated as final, stored as all-None). Quota logged as provider `ebay_item` in `api_quota_log`. AI relevance classifier (`ai_relevance.py`) gains new `offbrand` verdict: triggered when listing Brand does not match the search query's brand (e.g., search "Dorman OEM part" → listing Brand: Amk → offbrand). Rule: missing/Unbranded Brand never triggers offbrand. Dashboard tag for offbrand verdicts: `alt` (displayed alongside existing `acc` and `off` tags for accessory/unrelated). Database migration includes one-time verdict reset for all existing `search_listings` rows (`relevance` → NULL, `relevance_checked_at` → NULL) to trigger re-classification under the updated rules. Test suite: 178 tests passing. |
